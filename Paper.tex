...
In decision modeling the variability is described with decision instead of features. A variable feature in feature modeling is equal to a decision and vice versa. Contrary to feature modeling decision modeling is unable to describe the commonalities of systems.\\\\
Decision models can be represented in different ways, broadly used approaches are flat lists or tables of decisions which also includes their dependicies. In decision modeling hierarchy is a secondary aspect, because there are no uniform rules to describe hierarchy. \cite{b6}\\\\
Same as feature modeling, decision modeling has been expanded over time to enable the application coverage of DM be almost the same as FM.\cite{b6}\\\\
...
\section{Discussion/Challenges (1/4 page) JR}
The research field of variability had a increasing trend of published paper per year from 2000 until 2008. After that it stabilized between 25-30 new research paper per year from 2008 until 2011. One explanation of this rise in interest in the scientific community could be that the usage of variability was becoming more broadly in dynamic systems. \cite{b4}\\\\
Another aspect is that feature modeling is in its core a simple concept. Even though its impact in the industry is pretty big considering it is the most popular concept for variability modeling in software systesm until today.\cite{b9} One can say that because it is such a simple concept and due to the big amount of research in this field we understand the subject pretty well by now.\\\\
The currently existing methods also have their limitations. It is difficult to guarantee correctness and consistency, authors have to urge for verification and consistency checking. Performance overheads, runtime upgradability and the lack of support regarding security needs are some areas where the described methods are limited. Sometimes the handling of variability can create poor user-friendliness and therefore decrease usebility of a system. \cite{b4}\\\\
There are ways to decrease the negative impact of these aspects e.g. correctness and consistency can be checked by various methods such as modeling guidelines customized for specific needs, together with a reviewing process where changes to the model are checked by another person to ensure the quality of the metioned parameters.
...
(1/4 page) JR
>>>>>>> 07d0f30ddc607b1a81093bc76ccea31ca118cca3

\section{Conclussion (1/4 page) NH}
In this document we have given a short introduction to the origin of variability, how to model it and what patterns exist as well as look over related work to these topics. \\

This information is vital to being able to manage variability, it does however, only glance over the different subjects and only provides a theoretical basis. Practical use also depends on supporting software, which widely exists especially for the task of variability modelling, as well as practical knowledge in the field, which also aids in gaining an in depth understanding of managing variability.\\
