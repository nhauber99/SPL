...
\begin{abstract}
With the growing complexity of software systems, variability of said systems has become an increasing problem and managing it is thus a crucial part of the present-day software engineering process. It is particularly vital if either many product versions make use of an existing architecture over a long period of time or if a variety of software systems share a common code base. The focus of this paper is to provide an overview of how variation in software systems is caused, how it influences the development process and how it can be dealt with.
\end{abstract}
...
We will try to provide a basic introduction of how variability is managed in software systems by collecting information from other studies which describe the key aspects of this vast field of research. In the first section we are providing some base knowledge acquired from previous studies. After that we will lay out how one can identify the various sources of variability in software systems. Then we will bring up techniques and processes which have the purpose of modeling the variation in a software system. Last but not least we will discuss some of the big tasks which have to be addressed and conclude the topic.
...
The essence of feature modeling is the splitting of features into distinct objects which have a certain relation to each other. There are several types of features mentioned in \cite{b8}, named \textit{mandatory, optional and alternative features}. Generally speaking common attributes of systems are described by mandatory features, while alternative or optional features define the variability in feature models. Additionally so called cross tree constraints can be defined to model, for example mutual exclusive features across the whole tree structure.\cite{b9}\\

If a feature is mandatory it has to be selected if its parent feature is selected, the type of the parent feature does not matter. An optional feature can be selected freely but there may be constraints from composition rules (e.g. a feature can only be choosen as part of a feature sets). Alternative features are can be pictured as a choice box, exactly \textit{one} item must be chosen from a feature set.\\
...
Decision models can be represented in different ways, broadly used approaches are flat lists or tables of decisions which also includes their dependencies. In decision modeling hierarchy is a secondary aspect, because there are no uniform rules to describe hierarchy.
...
As mentioned in \cite{b11} the  different approaches for decision modeling have some common core model structure. There is an entity which is called decision, which represents a set of all possible choices which are available at a certain point in time.\\

These decision have attributes which can vary from one modeling approach to another. But commonly at least a decision name or identifier are used, while the other atrributes are describing the decision in such a way that the user can understand it. Furthermore the decision has a data type which defines the set of valid choices for decision.\cite{b11}\\

With all of the different approaches it is possible to create dependencies between decisions. There are value contraints for defining dependencies between values, which are supported by most approaches. But here it has to be mentioned that all the approaches use different concepts for value contraints and generally speaking for the management of the dependencies.\cite{b11}\\ 

Some of them use contraints, similar to the approach used in feature modeling. Another approach for modeling dependencies is with rules or decisions, which are are checked when a decision is taken.\cite{b11} \\

The second approach could be more intuitive for most people with a software engineering background because of the similarity how a computer program with conditional statements works.

\begin{figure}[h!]
	\includegraphics[width=\columnwidth]{decision_model}
	\caption{Simplified decision model of a car}
\end{figure}
Fig. 2 uses the same model as Fig. 1 but this time with the decision modeling approach was used. As described previously the decision model does not contain information about the commonalities in the system.\\
...
\begin{itemize}
	...
	\item \textit{Optional Entity.} An optional entity may or may not be included in the system.
...
Another aspect is that feature modeling is in its core a simple concept. Even though its impact on the industry is enormous considering it is the most popular concept for variability modeling in software system as of now.\cite{b9} \\
...
\item \textit{Lack of support for security needs} can be a factor which limits the use of feature modeling.
...
It should also be noted most of the studies in the area of FM include some sort of tool to support the process. Only a few don't mention any form of software support.\\

\section{Conclusion}
In this document we have given a short introduction to managing variability in software systems. We do this by establishing the origin of variability, how to model it with either feature modeling or decision modeling as well as what patterns of variability exist in addition to looking over work related to these topics and finally presenting the challenges. \\

Variability is intruduced early in the developement of a new product and can have four main origins: variation in function, variation in data, variation in technology and variation in environment.
This variability can then be modelled via either one of the main approaches, namely feature modeling or decision modeling.
The modelled variability mainly fall in three categories and can vary its binding time as described in section 5.\\
...
By now the scientific community has a solid understanding of variability in software systems and variability modelling has become a broadly used tool. 

\begin{thebibliography}{00}
\bibitem{b2} J. van Gurp, J. Bosch and M. Svahnberg, "On the notion of variability in software product lines," Proceedings Working IEEE/IFIP Conference on Software Architecture", Amsterdam, Netherlands, 2001
...
\bibitem{b5} T. Berger, J.-P. Steghöfer, T. Ziadi, J. Robin, and J. Martinez, "The state of adoption and the challenges of systematic variability management in industry", Empirical Software Engineering, vol. 25, 2020
\bibitem{b6}
...
\bibitem{b10} M. Riebisch, K. Böllert, D. Streitferdt, and I. Philippow, "Extending Feature Diagrams with UML Multiplicities", \textit{6th World Conference on Integrated Design \& Process Technology (IDPT2002)}, vol. 23, 2002
\bibitem{b11} K. Schmid, R. Rabiser, P. Grünbacher, "A comparison of decision modeling approaches in product lines", Proc. of the \textit{Fifth international
workshop on variability modeling of software-intensive systems
(VaMoS '11)}, ACM, Jan 2011
\end{thebibliography}
